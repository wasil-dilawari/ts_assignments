...
let exit = false;
let newUser = true;
const welcomeMessage = async (oldUser = false) => {
    console.clear();
...
const registerUser = async () => {
    ...
    do {
        ...
        if (newPass.newPassword1 === newPass.newPassword2)
            ...
            console.log("Pin Codes do not match. Please try again ...");
...
const atmOperation = async (username) => {
...
const cashWithdrawal = async () => {
    ...
    var withdrawalAmount = await inquirer.prompt([
        {
            name: "amount",
            message: "Please Enter the Amount you want to withdraw from your account:",
            type: "number",
            validate(input) {
                if (isNaN(input)) {
                    ...
                    console.log("Amount enter exceeds available Balance. Please enter valid Amount");
                }
                else
                    return true;
            },
